...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        if mask is not None:
            # mask 차원을 scores에 맞게 확장 (batch, 1, 1, seq_len) 또는 (batch, 1, seq_len, seq_len)
            if mask.dim() == 2:
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, src_mask):
        ...
        x = self.norm2(x + self.dropout(_x)) # norm2로 수정
        return x
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, enc_output, src_mask, trg_mask):
        ...
        # 2. Cross-Attention
        _x = self.cross_attn(x, enc_output, enc_output, src_mask)
...
class Transformer(nn.Module):
    def __init__(self, src_vocab_size, trg_vocab_size, d_model=512, num_layers=6, num_heads=8, d_ff=2048, dropout=0.1):
        super().__init__()
        ...
        # Weight Tying (노트북 메모리 효율 증가)
        self.fc_out.weight = self.decoder_emb.weight
    
    def forward(self, src, trg, src_mask, trg_mask):
        enc = self.dropout(self.pos_enc(self.encoder_emb(src) * math.sqrt(self.d_model)))
        for layer in self.encoder_layers:
            enc = layer(enc, src_mask)
        
        dec = self.dropout(self.pos_enc(self.decoder_emb(trg) * math.sqrt(self.d_model)))
